...
class ImportGTFS(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config=None):
        ...
        self.addParameter(QgsProcessingParameterString("agency", self.tr("Agency to import"), multiLine=False))
        self.addParameter(
            QgsProcessingParameterString(
                "gtfs_date", self.tr("Date to import (YYYY-MM-DD)"), multiLine=False, defaultValue="2024-01-01"
            )
        )
        self.addParameter(
            QgsProcessingParameterBoolean(
                "map_match", self.tr("map matching of PT lines on the aequilibrae project network"), defaultValue=True
            )
        )
        self.addParameter(
            QgsProcessingParameterFile(
